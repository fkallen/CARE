...
#define SAFE_ACCESS
	...
	void ReadStorage::noMoreInserts(){
		...
		for(int readnum = 0; readnum < nSequences; readnum++){
			...
			sequencesflat.push_back(std::move(reverseComplSequences[vecnum].at(indexInVector)));
		}
		for(auto& vec : reverseComplSequences)
			vec.clear();
		...
		std::cout << "ReadStorage: found " << (2 * nSequences - sequencesflat.size()) << " duplicates\n";
		...
		for(int i = 0; i < nSequences; i++)
			reverseComplSequencepointers[i] = &sequencesflat[seqToSortedIndex[tmp[nSequences + i]]];
		...
		for(int i = 0; i < nSequences; i++){
			assert(*reverseComplSequencepointers[i] == tmp[nSequences + i] && "readstorage wrong sequence after dedup");
		}
