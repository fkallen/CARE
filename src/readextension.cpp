    ...
    #pragma omp parallel
    {
        ...
        while(!(readIdGenerator.empty())){
            ...
            auto handleMultiResult = [&](
                const ReadExtenderCpu::ExtendResult* result1, 
                const ReadExtenderCpu::ExtendResult* result2,
                read_number readId1,
                read_number readId2,
                int readLength1,
                int readLength2,
                ...
                if(result1 != nullptr){
                    ...
                    if(result1->mateHasBeenFound){
                        ...
                        if(result1->aborted){
                            if(result1->abortReason == ReadExtenderCpu::AbortReason::NoPairedCandidates
                                    || result1->abortReason == ReadExtenderCpu::AbortReason::NoPairedCandidatesAfterAlignment){

                                er.status1 = ExtendedReadStatus::CandidateAbort;
                            }else if(result1->abortReason == ReadExtenderCpu::AbortReason::MsaNotExtended){
                ...
                if(result2 != nullptr){
                    ...
                    if(result2->mateHasBeenFound){
                        ...
                        if(result2->aborted){
                            if(result2->abortReason == ReadExtenderCpu::AbortReason::NoPairedCandidates
                                    || result2->abortReason == ReadExtenderCpu::AbortReason::NoPairedCandidatesAfterAlignment){

                                er.status2 = ExtendedReadStatus::CandidateAbort;
                            }else if(result2->abortReason == ReadExtenderCpu::AbortReason::MsaNotExtended){
