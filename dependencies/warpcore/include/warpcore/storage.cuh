...
public:
        ...
        is_copy_(false)
    {
        if(capacity != 0)
        {
            ...
            {
                cudaError_t cudastatus = cudaMalloc(&store_, sizeof(T) * capacity_);
...
public:
        ...
        is_copy_(false)
    {
        if(capacity != 0)
        {
            ...
            {
                cudaError_t cudastatus1 = cudaMalloc(&keys_, sizeof(key_type)*capacity);
                cudaError_t cudastatus2 = cudaMalloc(&values_, sizeof(value_type)*capacity);
...
public:
        ...
        is_copy_(false)
    {
        if(capacity != 0)
        {
            ...
            if(helpers::available_gpu_memory() >= total_bytes)
            {
                cudaError_t cudastatus = cudaMalloc(&pairs_, sizeof(pair_t) * capacity);
...
public:
        ...
        is_copy_(false)
    {
            ...
            max_bucket_size_ <= handle_type::max_bucket_size())
        {
            ...
            if(helpers::available_gpu_memory() >= total_bytes)
            {
                cudaError_t cudastatus1 = cudaMalloc(&buckets_, sizeof(bucket_type) * capacity_);
                cudaError_t cudastatus2 = cudaMalloc(&next_free_bucket_, sizeof(index_type));
