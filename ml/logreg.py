...
def main(prefixes, effiles, test_index):
    ...
    process(anchor_map, LogisticRegressionCV, {"Cs":10, "cv":22, "n_jobs":22, "solver":"saga", "max_iter":2**10, "penalty":"l1", "verbose":5}, "_anchor.lr", test_index)
    # process(cands_map, LogisticRegression, {"solver":"sag", "max_iter":2**10, "penalty":"none", "verbose":5}, "_cands.lr", test_index)
...
def process(data_map, clf_t, clf_args, suffix, test_index):
    ...
    for i in range(len(data_map)):
        test_map = [data_map[i]]
        test_data = read_data(test_map)
        X_test, y_test = scaler.transform(test_data['atts']), test_data['class']
