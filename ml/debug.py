...
print(sys.argv[1:])
foo = [np.load(open(x, "rb"))['samples'] for x in sys.argv[1:]]

for i, train in enumerate(foo):
#     print(sys.argv[1+i], train[np.isnan(np.sum(train['atts'], axis=1))])
    print(sys.argv[1+i], train['atts'][:,8:12])
