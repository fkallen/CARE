...
def main(prefixes, effiles, test_index):
    ...
    process(anchor_map, 22, "_maxdepth_anchor", test_index)
    process(anchor_map, None, "_anchor", test_index)
    process(cands_map, 22, "_maxdepth_cands", test_index)
    process(cands_map, None, "_cands", test_index)

def process(data_map, max_depth, suffix, test_index):
    tqdm.write("### data sets: "+str(len(data_map))+"\n")
    tqdm.write("### leave index "+str(test_index)+" out:\n")
    train_map = list(data_map)
    test_map = [train_map.pop(test_index-1)]
    ...
    clf = RandomForestClassifier(n_jobs=22, max_depth=max_depth).fit(X_train, y_train)
    ...
    extract_forest(clf, str(test_index)+suffix+".rf")
