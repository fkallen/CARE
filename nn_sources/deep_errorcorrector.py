...
def train(train_path, model_path):
    features_total = np.load(train_path+'_X.bin.npy')
    labels_total = np.load(train_path+'_Y.bin.npy')
    ...
    with tf.Session() as sess:
        ...
        for it in tqdm(range(0,num_epochs*bootstrap.shape[0], batch_size)):
            ...
            if batch_count % print_interval == 0:
                ...
                plt.title('ROC curve')
                plt.legend(loc='best')
                plt.savefig("ROC_03-21-"+str(ckpt_count)+".png")
                plt.clf()
                saver = tf.train.Saver()
                save_path = saver.save(sess, model_path+"_"+str(ckpt_count))
                sess.run(training_init_op, feed_dict={x:bootstrap, y:labels_bootstrap})
                ckpt_count+=1
            batch_count+=1
        ...
        sess.run(validation_init_op, feed_dict={x:validate, y:labels_validate})
        validate_predictions = np.zeros([labels_validate.shape[0],1])
        for it in tqdm(range(0,validate.shape[0],test_size)):
            batch_predictions = sess.run(prediction, feed_dict={train_toggle:False})
            validate_predictions[it:it+batch_predictions.shape[0]] = batch_predictions
...
# simple MLP with batch-norm for now
net = tf.layers.batch_normalization(net, training=train_toggle) # initial input norm

# hidden layers
hidden_dims = [512,512,512]
for h in hidden_dims:
    net = tf.layers.dense(net, h, activation=prelu)
