...
__global__
void
cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_exp_improved2(
			int* alignment_scores,
			int* alignment_overlaps,
			int* alignment_shifts,
			int* alignment_nOps,
...
__global__
void
cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_exp_improved(
            int* alignment_scores,
            int* alignment_overlaps,
            int* alignment_shifts,
            int* alignment_nOps,
        ...
        // sizeof(char) * (max_sequence_bytes * blocksize + 2 * max_sequence_bytes);
        extern __shared__ unsigned int sharedmemory[];

        unsigned int* const constantSubject = (unsigned int*)(((char*)sharedmemory)); //sizeof(char) * max_sequence_bytes
        unsigned int* const constantQuery = (unsigned int*)(((char*)constantSubject) + max_sequence_bytes); //sizeof(char) * max_sequence_bytes
        unsigned int* const mySequence = ((unsigned int*)(((char*)constantQuery) + max_sequence_bytes)) + threadIdx.x; //sizeof(char) * max_sequence_bytes * BLOCKSIZE
        ...
        for(unsigned resultIndex = blockIdx.x; resultIndex < n_candidates * 2; resultIndex += gridDim.x) {
            ...
            for(int lane = threadIdx.x; lane < max_sequence_ints; lane += BLOCKSIZE) {
                constantSubject[lane] = ((unsigned int*)(subjectptr))[lane];
            }
            ...
            for(int lane = threadIdx.x; lane < max_sequence_ints; lane += BLOCKSIZE) {
                constantQuery[lane] = ((unsigned int*)(candidateptr))[lane];
            }

            //queryIndex != resultIndex -> reverse complement
            if(threadIdx.x == 0 && queryIndex != resultIndex) {
                make_reverse_complement_inplace(constantQuery, querybases);
            }
            ...
            //process positive shifts. positive shifts modify subject -> each thread needs its own copy of subject
            for(int lane = 0; lane < max_sequence_ints; lane += 1) {
                mySequence[no_bank_conflict_index(lane)] = constantSubject[lane];
            }
            ...
            for(int shift = threadIdx.x; shift < subjectbases - minoverlap + 1; shift += BLOCKSIZE){
                ...
                unsigned int* const shiftptr_hi = mySequence;
                unsigned int* const shiftptr_lo = mySequence + subjectints / 2 * BLOCKSIZE;
                const int size = subjectints / 2;
                const int shiftamount = previousShift == std::numeric_limits<int>::min() ?
                                        shift : shift - previousShift;
                //printf("thread %d shift %d previousshift %d shiftamount %d\n",
                //        threadIdx.x, shift, previousShift, shiftamount);
                ...
                int score = hammingdistanceHiLo(
                            ...
                            shiftptr_lo,
                            constantQuery,
                            constantQuery + queryints / 2 * BLOCKSIZE,
                            max(0, subjectbases - abs(shift)),
                            max(0, querybases - abs(shift)),
                            max_errors);
                ...
                score = (score < max_errors ?
                 ...
                 //printf("sub shift %d oldscore %d score %d bestScore %d max_errors %d\n",
                //         shift, oldscore, score, bestScore, max_errors);
            ...
            //process negative shifts. negative shifts modify query -> each thread needs its own copy of query
            for(int lane = 0; lane < max_sequence_ints; lane += 1) {
                mySequence[no_bank_conflict_index(lane)] = constantQuery[lane];
            }

            previousShift = std::numeric_limits<int>::min();

            for(int shift = -threadIdx.x; shift >= -querybases + minoverlap; shift -= BLOCKSIZE){
                const int overlapsize = min(querybases, subjectbases - shift) - max(-shift, 0);
                const int max_errors = int(double(overlapsize) * maxErrorRate);

                unsigned int* const shiftptr_hi = mySequence;
                unsigned int* const shiftptr_lo = mySequence + queryints / 2 * BLOCKSIZE;
                const int size = queryints / 2;
                const int shiftamount = previousShift == std::numeric_limits<int>::min() ?
                                        abs(shift) : abs(shift - previousShift);
                ...
                int score = hammingdistanceHiLo(
                            constantSubject,
                            constantSubject + subjectints / 2 * BLOCKSIZE,
                            shiftptr_hi,
                            shiftptr_lo,
                            max(0, subjectbases - abs(shift)),
                            max(0, querybases - abs(shift)),
                ...
                //printf("quer shift %d oldscore %d score %d bestScore %d max_errors %d\n",
                //        shift, oldscore, score, bestScore, max_errors);
            ...
            if(threadIdx.x == 0) {
                ...
                /*if(bestScore != 200)
                    printf("resultIndex %d bestScore %d bestShift %d\n",
                        resultIndex, bestScore, bestShift);*/
            }
        }
}
...
template<class B, class GetSubjectPtr, class GetCandidatePtr, class GetSubjectLength, class GetCandidateLength>
void call_cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_exp_improved_async(
			int* d_alignment_scores,
			int* d_alignment_overlaps,
			int* d_alignment_shifts,
			int* d_alignment_nOps,
	    ...
	    #define mycall(blocksize) cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_exp_improved2<(blocksize)> \
	        <<<grid, block, smem, stream>>>( \
    		d_alignment_scores, \
    		d_alignment_overlaps, \
    		d_alignment_shifts, \
    	...
    	auto iter = handle.kernelPropertiesMap.find(KernelId::PopcountSHDExpImproved);
    	if(iter == handle.kernelPropertiesMap.end()) {
    		...
    		handle.kernelPropertiesMap[KernelId::PopcountSHDExpImproved] = std::move(mymap);
    	...
    	case 224: mycall(224); break;
    	case 256: mycall(256); break;
    	default: throw std::runtime_error("Want to call cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_exp_improved with wrong blocksize due to a bug.");
    	}
...
cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_transposed(
	...
	for(unsigned resultIndex = blockIdx.x * BLOCKSIZE + threadIdx.x; resultIndex < n_candidates * 2; resultIndex += BLOCKSIZE * gridDim.x) {
		...
		for(int lane = 0; lane < max_sequence_ints; lane += 1) {
			myQuery[lane] = ((unsigned int*)(candidateptr))[lane];
		}
...
void call_cuda_popcount_shifted_hamming_distance_with_revcompl_kernel_transposed_async(
	...
	const std::size_t smem = sizeof(char) * (2 * max_sequence_bytes * blocksize);
	...
	if(iter == handle.kernelPropertiesMap.end()) {
		...
		KernelLaunchConfig kernelLaunchConfig; \
		kernelLaunchConfig.threads_per_block = (blocksize); \
		kernelLaunchConfig.smem = sizeof(char) * (2 * max_sequence_bytes * (blocksize)); \
		KernelProperties kernelProperties; \
		cudaOccupancyMaxActiveBlocksPerMultiprocessor(&kernelProperties.max_blocks_per_SM, \
