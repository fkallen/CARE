...
public:
    ...
    DistributedArray(std::vector<int> deviceIds_, std::vector<float> maxFreeMemFraction_, Index_t numElements_, size_t sizeOfElement_, int preferedLocation_ = -1)
                    : debug(false),
    ...
    void set(Index_t index, const char* data){
    ...
    void set(Index_t firstIndex, Index_t lastIndex_excl, const char* data){
    ...
    void set(const std::vector<Index_t>& indices, const char* data){
    ...
    void get(Index_t index, char* result){
        int location = getLocation(index);
        Index_t localIndex = index - elementsPerLocationPS[location];
        if(location == hostLocation){
    ...
    void gatherElementsInHostMem(const GatherHandle& handle,
        ...
        for(Index_t dstindex = 0; dstindex < numIds; dstindex++){
            ...
            std::copy_n(&handle->pinnedResultData[srcindex * sizeOfElement], sizeOfElement, &result[dstindex * resultPitch]);
        }
    }
...
*/
                    ...
                    Index_t* localIdsPtr = handle->localIndicesOnDevice[deviceId].get() + hitsPerLocationPrefixSum[gpu];
                    //copyDataToGpuBufferAsync(destptr, deviceId, myLocalIds.get(), numHits, mydeviceId, stream, 0);
                    copyDataToGpuBufferAsync(destptr, sizeOfElement, deviceId, localIdsPtr, numHits, mydeviceId, stream, 0);
                }else{
        ...
        //gather from host to host
        if(hitsPerLocation[hostLocation] > 0){
            Index_t numHits = hitsPerLocation[hostLocation];
            const Index_t* hostLocalIds = handle->pinnedLocalIndices.get() + hitsPerLocationPrefixSum[hostLocation];
            char* hostResult = handle->pinnedResultData.get() + hitsPerLocationPrefixSum[hostLocation] * sizeOfElement;
            for(Index_t k = 0; k < numHits; k++){
                Index_t localId = hostLocalIds[k];
                std::copy_n(&dataPtrPerLocation[hostLocation][localId * sizeOfElement], sizeOfElement, &hostResult[k * sizeOfElement]);
            }
        }
...

        {
            ...
            const Index_t* indices = handle->permutationIndicesOfDevice[deviceId].get();
            ...
            generic_kernel<<<grid, block, 0, stream>>>([=] __device__ (){
                for(Index_t i = threadIdx.x + Index_t(blockIdx.x) * blockDim.x; i < n; i += Index_t(blockDim.x) * gridDim.x){
            	    ...
            	    for(size_t b = 0; b < size; b++){
                        ...
                        dest[i * resultPitch + b] = src[srcindex * size + b];
                    }
                }
            });
    ...
    void copyDataToGpuBufferAsync(char* d_result, size_t resultPitch, const Index_t* d_indices, Index_t nIndices, int deviceId, cudaStream_t stream, Index_t indexOffset) const{
        assert(resultPitch >= sizeOfElement);
        ...
        generic_kernel<<<grid, block,0, stream>>>([=] __device__ (){
            ...
            for(Index_t k = threadIdx.x + Index_t(blockIdx.x) * blockDim.x; k < nIndices; k += Index_t(blockDim.x) * gridDim.x){
                ...
                // for(int b = 0; b < intiters; b++){
                //     ((int*)&d_result[k * resultPitch])[b] = ((int*)&gpuData[index * sizeOfElement_])[b];
                // }
                // for(size_t b = intiters * sizeof(int); b < resultPitch; b++){
    ...
    void copyDataToGpuBufferAsync(char* d_result, size_t resultPitch, int resultDevice, const Index_t* d_indices, Index_t nIndices, int sourceDevice, cudaStream_t stream, Index_t indexOffset) const{
        assert(resultPitch >= sizeOfElement);
        int oldDevice; cudaGetDevice(&oldDevice); CUERR;
    ...
    void copyDataToGpuBufferAsync(char* d_result, size_t resultPitch, const Index_t* d_indices, Index_t nIndices, int deviceId, cudaStream_t stream) const{
...
//TIMERSTARTCPU(resizedevicevectors)
        ...
        handle->tmpResultsOfDevice[deviceId].resize(numIds * numColumns);
...
*/
                    cudaSetDevice(deviceId); CUERR;
                    //cudaStreamWaitEvent(stream, myevent,0); CUERR; //wait in result stream until remote indices are ready
                    Value_t* destptr = offsetPtr(handle->tmpResultsOfDevice[deviceId].get(), hitsPerLocationPrefixSum[gpu]);
                    Index_t* localIdsPtr = handle->localIndicesOnDevice[deviceId].get() + hitsPerLocationPrefixSum[gpu];
                    //copyDataToGpuBufferAsync(destptr, deviceId, myLocalIds.get(), numHits, mydeviceId, stream, 0);
                    copyDataToGpuBufferAsync(destptr, sizeOfElement, deviceId, localIdsPtr, numHits, mydeviceId, stream, 0);
                }else{
...

        {
            ...
            const Index_t* indices = handle->permutationIndicesOfDevice[deviceId].get();
            const Value_t* src = handle->tmpResultsOfDevice[deviceId].get();
