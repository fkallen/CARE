...
/*
    Calculates ONE result per query which is the best alignment out of forward alignment and reverse complement alignment
*/

template<int BLOCKSIZE, class Accessor, class AlignmentComp>
__global__
void
cuda_shifted_hamming_distance_canonical_kernel(Result_t* results,
    ...
    for(unsigned queryIndex = blockIdx.x; queryIndex < NqueriesPrefixSum[Nsubjects]; queryIndex += gridDim.x){
        ...
        //begin SHD algorithm
        Result_t fwdAlignment;
        Result_t revComplAlignment;

        //for forward query and revcompl query
        for(int i = 0; i < 2; i++){

            const char* query = i == 0 ? sharedQuery : sharedQueryRevcompl;
            Result_t* resultptr = i == 0 ? &fwdAlignment : &revComplAlignment;
            ...
            if(threadIdx.x == 0){
                ...
                resultptr->isValid = (bestShift != -querybases);
                const int queryoverlapbegin_incl = max(-bestShift, 0);
                const int queryoverlapend_excl = min(querybases, subjectbases - bestShift);
                const int overlapsize = queryoverlapend_excl - queryoverlapbegin_incl;
                const int opnr = bestScore - totalbases + 2*overlapsize;

                resultptr->score = bestScore;
                resultptr->subject_begin_incl = max(0, bestShift);
                resultptr->query_begin_incl = queryoverlapbegin_incl;
                resultptr->overlap = overlapsize;
                resultptr->shift = bestShift;
                resultptr->nOps = opnr;
                resultptr->isNormalized = false;
            }
        }

        if(threadIdx.x == 0){
            BestAlignment_t bestAlignmentFlag = comp(fwdAlignment, revComplAlignment, subjectbases, querybases);
            bestAlignmentFlags[queryIndex] = bestAlignmentFlag;

            if(bestAlignmentFlag == BestAlignment_t::Forward)
                results[queryIndex] = fwdAlignment;
            else if(bestAlignmentFlag == BestAlignment_t::ReverseComplement)
                results[queryIndex] = revComplAlignment;
            else
...
template<class Accessor, class AlignmentComp>
void call_shd_canonical_kernel_async(const SHDdata& shddata,
                      int min_overlap,
                      double maxErrorRate,
                      double min_overlap_ratio,
                      int maxSubjectLength,
      ...
      #define mycall(blocksize) cuda_shifted_hamming_distance_canonical_kernel<(blocksize)> \
                                  <<<grid, block, smem, shddata.streams[0]>>>( \
                                  shddata.d_results, \
...
template<class Accessor, class AlignmentComp>
void call_shd_canonical_kernel(const SHDdata& shddata,
                      int min_overlap,
                      double maxErrorRate,
                      double min_overlap_ratio,
                      int maxSubjectLength,
    ...
    call_shd_canonical_kernel_async(shddata,
                        min_overlap,
                        maxErrorRate,
                        min_overlap_ratio,
                        maxSubjectLength,
